...
<div class="page-header">
    <ul class="breadcrumbs ps-1 ms-0">
        ...
        <li class="nav-item">
            <a href="<?= base_url('tecnico/dispositivos/pool') ?>">Pool de Dispositivos</a>
        </li>
        <li class="separator"><i class="icon-arrow-right"></i></li>
...
<script>
    ...
    function recalcularRepuesto(index) {
        const container = document.getElementById('lista-repuestos-' + index);
        let total = 0;
        Array.from(container.children).forEach((wrapper, wi) => {
            const item = wrapper.querySelector(`#repuesto-${index}-${wi}`);
            if (!item) return;
    ...
    function iniciarReparacion(id) {
        ...
        const title = !tecnicoIdAsignado ? 'Tomar Reparación' : '¿Iniciar reparación?';
        const text = !tecnicoIdAsignado
            ? 'No hay técnico asignado. Al iniciar, quedarás como responsable.'
            : 'El dispositivo pasará a estado "En Proceso".';
        const icon = !tecnicoIdAsignado ? 'info' : 'question';

        Swal.fire({
            title, text, icon, showCancelButton: true,
            confirmButtonText: 'Sí, iniciar', cancelButtonText: 'Cancelar',
            ...
            Swal.fire({
                ...
                <div class="text-start">
                    <label class="form-label small fw-bold text-muted text-uppercase">
                        Motivo para el Cliente <span class="text-danger">*</span>
                    </label>
                    <textarea id="swal-comentario" class="swal2-textarea w-100"
                              placeholder="Ej: No se consiguió el repuesto..."></textarea>
                    <label class="form-label small fw-bold text-muted text-uppercase mt-2">
                        Nota Técnica Interna
                    </label>
                    <textarea id="swal-nota-tecnica" class="swal2-textarea w-100"
                              placeholder="Ej: Se intentó reparar X pero falló Y..."></textarea>
                </div>`,
                icon: 'warning',
                ...
                fetch('<?= base_url('tecnico/dispositivos/reparacion/finalizar') ?>', { method: 'POST', body: fd })
                    .then(r => r.json())
                    .then(data => {
