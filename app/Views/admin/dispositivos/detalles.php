        ...
        <div class="dv-card">
            ...
            <div class="dv-action-area">
                ...
                <?php elseif ($estado === 'en_proceso'): ?>
                    <button class="dv-btn-action btn-finish" data-bs-toggle="modal" data-bs-target="#modalFinalizar"
                        onclick="validarFinalizacion(event)">
...
<script>
    ...
    /* ── Validación antes de abrir modal finalizar ── */
    function validarFinalizacion(e) {
        ...
        if (tecnicoIdAsignado != currentUserId && userRole !== 'admin') {
            e.preventDefault();
            e.stopPropagation();
            showAlert('error', 'No eres el técnico asignado. No puedes finalizar esta reparación.', 'center');
        }
    }
    ...
    function iniciarReparacion(id) {
        ...
        if (tecnicoIdAsignado && tecnicoIdAsignado != currentUserId) {
            if (userRole !== 'admin') { showAlert('error', 'Este dispositivo está asignado a otro técnico.', 'center'); return; }
            title = 'Tomar Reparación'; text = 'Está asignado a otro técnico. Se te re-asignará a ti.'; icon = 'warning';
        } else if (!tecnicoIdAsignado) {
            title = 'Asignación Automática'; text = 'No hay técnico asignado. Quedarás como responsable.'; icon = 'info';
        }

        Swal.fire({
            title, text, icon, showCancelButton: true, confirmButtonText: 'Sí, continuar', cancelButtonText: 'Cancelar',
    ...
    function entregarDispositivo(id) {
        Swal.fire({
            title: '¿Entregar al cliente?', text: 'Esta acción marcará el dispositivo como entregado.', icon: 'question',
            showCancelButton: true, confirmButtonText: 'Confirmar', cancelButtonText: 'Cancelar', confirmButtonColor: '#0ea5e9'
        })
            .then(res => {
                if (!res.isConfirmed) return;
